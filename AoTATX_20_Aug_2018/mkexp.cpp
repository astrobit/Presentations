...
{
	fprintf(fileOut,"\\draw[yellow] (6.4,%.2f) node {%s};\n",dY+0.75,lpszString);
	fprintf(fileOut,"\\begin{scope}[even odd rule]\n");
	fprintf(fileOut,"\\clip (1,%.2f) circle (0.45);\n",dY);
	fprintf(fileOut,"\\tkzpic{0.5}{%.2f}{1}{1}{Pictures/tycho};\n",dY-0.5);
	fprintf(fileOut,"\\end{scope}\n");
	fprintf(fileOut,"\\begin{scope}[even odd rule]\n");
	fprintf(fileOut,"\\clip (11.5,%.2f) circle (0.25);\n",dY);
	fprintf(fileOut,"\\tkzpic{11.25}{%.2f}{0.5}{0.5}{Pictures/115334main_image_feature_329_ys_full};\n",dY-0.25);
	fprintf(fileOut,"\\end{scope}\n");
}
