...
\begin{tabular}{l}
Add mass to make gravity overcome pressure (how?)\\
At 1.4$\;\mathrm{M}_{\odot}$ gravity starts win\\
...
\foreach \x in {1,...,30}{
		\coordinate (A) at ($ (2,5.5) + (-90+rand*15.0:rand*1.5)$);
...
\draw[yellow] (7,2.75) node[left] {Photospheric-Velocity Feature (PVF)};
\draw[yellow] (15,2.75) node[left] {High-Velocity Feature (HVF)};
...
%\begin{tabular}{l}
%Can HVF give insight into nature of system or cause of the explosion?
%\\
%\\
...
Clumps of high velocity material ejected from SN\\
\color{magenta}\hspace{2em} Explains polarization\\
\color{magenta}\hspace{2em} Likely be greatly enhanced in freshly synthesized elements\\
\color{magenta}\hspace{2em} No obvious explanation for velocity evolution\\
...
\fill[white] (3.1,1.0) rectangle +(0.86,7.7);
...
\begin{tikzpicture}
\draw[opacity=0] (0.1,0.1) rectangle (15.8,9);
\draw[white] (5.0,8.75) node {\textsc{syn++} (Thomas et al. 2011)};
\draw[white] (5.0,7.75) node[right] {Allows selection and control of individual ions};
\draw[white] (5.0,8.25) node[right] {specify $T_{\mathrm{PS}}$, $v_{\mathrm{PS}}$, $T_{\mathrm{exc}}$};
...
\hspace{4em} Too much Ca in ejecta at high velocity?\\
...
\begin{tabular}{l}
\color{white}Why does the Ca content need to increase with time?\\
...
\hspace{2em} Interaction done in seconds -- no excess radiation at epochs when SNe are observed.\\
\hspace{2em} Velocity of contact discontinuity $\sim$15 -- 25,000\kmsp.\\
\hspace{2em} Maximum velocity of shell $\sim$ 100,000\kmsp.
\hspace{2em} Mass of $\sim$0.01\Msun; 0.005\Msun explains velocity of HVF at $>$ 1 week after explosion.
\hspace{2em} Requires Solar -- super-Solar calcium abundance.\\
\hspace{2em} Requires super-Solar silicon abundance?\\
\hspace{2em} Substrate unclear -- won't show up in spectra.\\
\hspace{2em} If shell is associated with helium envelope \& detonation, needs to be $\sim$0.008\Msunp.\\
\end{tabular}
}
